...
const GetFinanceBanner = dynamic(() => import('../page-section/GetFinance/GetFinanceBanner'));
const GetFinanceService = dynamic(() => import('../page-section//GetFinance/GetFinanceService'));
const GetFinancePurpose = dynamic(() => import('../page-section/GetFinance/GetFinancePurpose'));
const GetFinanceQuick = dynamic(() => import('../page-section/GetFinance/GetFinanceQuick'));


export default function GetFinanced() {
  const classes = useStyles();
  const { locale } = useRouter();
  const { t } = useTranslation('common');
  const canonicalUrl = (
    BaseUrl + `${locale}/getFinanced`
  );
  const router = useRouter();
  useEffect(() => {
      ReactGA.send({ hitType: "pageview", page: "/getFinanced",});
    }, [router]);
  ...
  return (
    <div className={classes.mainSection}>
      <Head>
        <title>Get Financed | رقمية - Crowdfunding in Saudi Arabia | SME Crowd Lending in Saudi Arabia</title>
        <meta charSet="utf-8" />
